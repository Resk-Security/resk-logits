    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if input_ids.shape[1] > 0:
            for batch_idx in range(batch_size):
                ...
                self.states[batch_idx] = self.ac.step(
...
class MultiLevelShadowBanProcessor(ShadowBanProcessor):
    ...
    def __init__(
        ...
        for level, phrases in banned_phrases_by_level.items():
            if phrases:
                self.automatons[level] = VectorizedAhoCorasick(
                    tokenizer, phrases, device
                )

        # State tracking per level
        self.states = {level: {} for level in self.automatons.keys()}
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for level in self.automatons.keys():
            for batch_idx in range(batch_size):
                if batch_idx not in self.states[level]:
                    self.states[level][batch_idx] = 0
        ...
        if input_ids.shape[1] > 0:
            for batch_idx in range(batch_size):
                ...
                for level, ac in self.automatons.items():
                    self.states[level][batch_idx] = ac.step(
                        self.states[level][batch_idx],
                        last_token
                    )
        ...
        if 'high' in self.automatons:
            ...
            for batch_idx in range(batch_size):
                if high_ac.has_match(self.states['high'][batch_idx]):
                    scores[batch_idx, :] = -float('inf')
                    scores[batch_idx, self.tokenizer.eos_token_id] = 0.0
